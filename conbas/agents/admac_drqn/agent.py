...
from .model import AdmacDrqn, ADMAC
...
class AdmacDrqnAgent:
    ...
    def __init__(self, config: Dict[str, Any], commands: List[str], word_vocab: List[str]) -> None:
        ...
        self.commands_id = [words_to_ids(preproc(c), self.word2id) for c in self.commands]
        self.cmds, self.cmds_length = self.pad_input_ids(self.commands_id)
    ...
    def q_values(self, input_tensor: torch.Tensor, input_lengths: torch.Tensor,
        ...
        """
        state_representations = lstm_dqn_model.embedding(input_tensor, input_lengths)
    ...
    def act(self, input_ids: List[List[int]],
        ...
        with torch.no_grad():
            n_batch = input_tensor.size(1)

            # n_batch, embedding
            state_embed = self.lstm_dqn.embedding(input_tensor, input_lengths)
            # n_batch, n_cmds, embedding
            state_embed = state_embed.unsqueeze(1).repeat(1, len(self.commands), 1)
            # n_cmds, embedding
            cmds_embed = self.lstm_dqn.embedding(self.cmds, self.cmds_length)
            # n_batch, n_cmds, embedding
            cmds_embed = cmds_embed.unsqueeze(dim=0).repeat(n_batch, 1, 1)

            # n_batch, n_cmds
            logits = self.admac(state_embed, cmds_embed).squeeze(-1)
            admissible_mask = torch.sigmoid(logits).round().long()
            q_values[admissible_mask] = torch.tensor(float('-inf'))
    ...
    def train(self, env: gym.Env, config_file: str) -> None:
        ...
        optimizer_admac = optim.Adam(filter(lambda p: p.requires_grad,
                                            self.admac.parameters()), lr=train_config["optimizer"]["lr"])
    ...
    def update(self,
               ...
               optimizer_admac: optim.Optimizer,
        ...
        for i, batch in enumerate(sequence_batch):
            ...
            with torch.no_grad():
                next_state_embed = self.lstm_dqn.embedding(next_input_tensor, next_input_lengths)
            ...
            with torch.no_grad():
                ...
                with torch.no_grad():
                    cmds_embed = self.lstm_dqn.embedding(self.cmds, self.cmds_length)
                # n_batch, n_cmds
                logits = self.admac(state_embed, cmds_embed[command_indices])
        ...
        adm_loss = torch.stack(losses).mean()
        ...
        # update step
        optimizer.zero_grad()
        optimizer_admac.zero_grad()

        loss.backward(retain_graph=True)
        total_norm = clip_grad_norm_(self.lstm_dqn.parameters(), clip_grad_norm)

        adm_loss.backward(retain_graph=False)

        optimizer.step()
        optimizer_admac.step()
